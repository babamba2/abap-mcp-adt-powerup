...
exports.parseEnhancementsFromXml = parseEnhancementsFromXml;
exports.handleGetEnhancements = handleGetEnhancements;
 ...
 */
function parseEnhancementsFromXml(xmlData) {
    const enhancements = [];
    try {
        ...
        let match = sourceRegex.exec(xmlData);
        let index = 0;
        while (match !== null) {
            const enhancement = {
            ...
            if (base64Source) {
                try {
                    ...
                    utils_1.logger?.warn(`Failed to decode source code for enhancement ${enhancement.name}:`, decodeError);
        ...
        utils_1.logger?.info(`Parsed ${enhancements.length} enhancement implementations`);
        return enhancements;
    }
    catch (parseError) {
        utils_1.logger?.error('Failed to parse enhancement XML:', parseError);
        return [];
    }
}
/**
 ...
 */
async function determineObjectTypeAndPath(connection, objectName, manualProgramContext) {
    try {
        // First try as a class
        const classUrl = `/sap/bc/adt/oo/classes/${(0, utils_1.encodeSapObjectName)(objectName)}`;
        try {
            const response = await (0, utils_1.makeAdtRequestWithTimeout)(connection, classUrl, 'GET', 'csrf', {
                Accept: 'application/vnd.sap.adt.oo.classes.v4+xml',
            });
            if (response.status === 200) {
                utils_1.logger?.info(`${objectName} is a class`);
                return {
                    type: 'class',
                    basePath: `/sap/bc/adt/oo/classes/${(0, utils_1.encodeSapObjectName)(objectName)}/source/main/enhancements/elements`,
                };
            }
        }
        catch (_classError) {
            // If class request fails, try as program
            utils_1.logger?.info(`${objectName} is not a class, trying as program...`);
        }
        // Try as a program
        const programUrl = `/sap/bc/adt/programs/programs/${(0, utils_1.encodeSapObjectName)(objectName)}`;
        try {
            const response = await (0, utils_1.makeAdtRequestWithTimeout)(connection, programUrl, 'GET', 'csrf', {
                Accept: 'application/vnd.sap.adt.programs.v3+xml',
            });
            if (response.status === 200) {
                utils_1.logger?.info(`${objectName} is a program`);
                return {
                    type: 'program',
                    basePath: `/sap/bc/adt/programs/programs/${(0, utils_1.encodeSapObjectName)(objectName)}/source/main/enhancements/elements`,
                };
            }
        }
        catch (_programError) {
            // If program request fails, try as include
            utils_1.logger?.info(`${objectName} is not a program, trying as include...`);
        }
        // Try as include
        const includeUrl = `/sap/bc/adt/programs/includes/${(0, utils_1.encodeSapObjectName)(objectName)}`;
        const response = await (0, utils_1.makeAdtRequestWithTimeout)(connection, includeUrl, 'GET', 'csrf', {
            Accept: 'application/vnd.sap.adt.programs.includes.v2+xml',
        });
        if (response.status === 200) {
            utils_1.logger?.info(`${objectName} is an include`);
            let context;
            // Use manual program context if provided
            if (manualProgramContext) {
                context = `/sap/bc/adt/programs/programs/${manualProgramContext}`;
                utils_1.logger?.info(`Using manual program context for include ${objectName}: ${context}`);
            }
            else {
                ...
                if (contextMatch?.[1]) {
                    ...
                    utils_1.logger?.info(`Found auto-determined context for include ${objectName}: ${context}`);
                }
                else {
                    throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, `Could not determine parent program context for include: ${objectName}. No contextRef found in metadata. Consider providing the 'program' parameter manually.`);
                }
            }
            return {
                type: 'include',
                basePath: `/sap/bc/adt/programs/includes/${(0, utils_1.encodeSapObjectName)(objectName)}/source/main/enhancements/elements`,
                context: context,
            };
        }
        throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, `Could not determine object type for: ${objectName}. Object is neither a valid class, program, nor include.`);
    }
    catch (error) {
        if (error instanceof utils_1.McpError) {
            throw error;
        }
        utils_1.logger?.error(`Failed to determine object type for ${objectName}:`, error);
        throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, `Failed to determine object type for: ${objectName}. ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
...
function parseIncludesFromXml(xmlData) {
    ...
    try {
        ...
        utils_1.logger?.warn('Error parsing XML for includes:', error);
    }
    return includes;
}
/**
...
function parseIncludeNamesFromXml(xmlData) {
    ...
    try {
        ...
        utils_1.logger?.warn('Error parsing XML for include names:', error);
    }
 ...
 * @returns Array of include names
 */
async function getIncludesListInternal(context, objectName, objectType) {
    ...
    try {
        ...
        const rootResponse = await Promise.race([
            ...
            true),
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout: Failed to get root node structure for ${objectName} within 10000ms`)), 10000)),
        ]);
        ...
        const includesResponse = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout: Failed to get includes list for ${objectName} within 10000ms`)), 10000)),
        ]);
 ...
 */
async function getEnhancementsForSingleObject(connection, objectName, manualProgramContext) {
    utils_1.logger?.info(`Getting enhancements for single object: ${objectName}`, manualProgramContext
    ...
    if (objectInfo.type === 'include' && objectInfo.context) {
        ...
        utils_1.logger?.info(`Using context for include: ${objectInfo.context}`);
    }
    utils_1.logger?.info(`Final enhancement URL: ${url}`);
    ...
    if (response.status === 200 && response.data) {
        ...
        throw new utils_1.McpError(utils_1.ErrorCode.InternalError, `Failed to retrieve enhancements for ${objectName}. Status: ${response.status}`);
    }
}
/**
 * Filters enhancement response to show minimal information
 */
function filterMinimalEnhancements(response) {
    if (response.objects) {
        // For nested results, filter each object
        const filteredObjects = response.objects
            .filter((obj) => obj.enhancements && obj.enhancements.length > 0) // Only objects with enhancements
            .map((obj) => ({
            object_name: obj.object_name,
            object_type: obj.object_type,
            enhancements: obj.enhancements.map((enh) => ({
                name: enh.name,
                type: enh.type,
 ...
 */
async function handleGetEnhancements(context, args) {
    const { connection, logger } = context;
    try {
        ...
        const processNestedIncludes = async () => {
            ...
            for (const includeName of includesList) {
                try {
                    ...
                    const includeEnhancements = await createPromiseWithTimeout(includeEnhancementsPromise, requestTimeout, `Timeout: Failed to get enhancements for include ${includeName} within ${requestTimeout}ms`);
        ...
        try {
            ...
            if (args.filePath) {
                (0, writeResultToFile_1.writeResultToFile)(fallbackResult, args.filePath);
            }
 ...
 */
function createPromiseWithTimeout(promise, timeoutMs, errorMessage) {
    let timeoutId;
...
//# sourceMappingURL=handleGetEnhancements.js.map