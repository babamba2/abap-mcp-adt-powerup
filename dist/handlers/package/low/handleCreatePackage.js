...
exports.TOOL_DEFINITION = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            record_changes: {
                type: 'boolean',
                description: 'Enable change recording for the package. Required for transportable packages (non-$TMP). Default: false.',
            },
...
async function handleCreatePackage(context, args) {
    ...
    try {
        const { package_name, super_package, description, package_type, software_component, transport_layer, transport_request, record_changes, application_component, session_id, session_state, } = args;
        // Validation
        if (!package_name || !super_package || !description) {
            return (0, utils_1.return_error)(new Error('package_name, super_package, and description are required'));
        }
        ...
        try {
            ...
            if (error.message?.includes('Refresh token has expired') ||
                ...
                return (0, utils_1.return_error)(new Error(`Authentication failed: ${error.message}. Please re-authenticate using the authentication tool or update your credentials.`));
            }
            ...
            if (error.response?.status === 401 || error.response?.status === 403) {
                ...
                return (0, utils_1.return_error)(new Error(authError));
            }
            // Parse error message
