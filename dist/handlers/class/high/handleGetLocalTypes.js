...
exports.handleGetLocalTypes = handleGetLocalTypes;
...
async function handleGetLocalTypes(context, args) {
    ...
    try {
        ...
        try {
            ...
            const readResult = await localTypes.read({ className }, version);
            ...
            logger?.warn(`GetLocalTypes failed (HTTP ${status ?? 'unknown'}) for ${className}${url ? ` at ${url}` : ''}`, responseSnippet ? { response: responseSnippet } : undefined);
            logger?.error(`Error reading local types for ${className}: ${error?.message || error}`);
...
//# sourceMappingURL=handleGetLocalTypes.js.map