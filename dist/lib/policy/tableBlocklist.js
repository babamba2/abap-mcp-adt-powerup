...
exports.formatAskPrompt = formatAskPrompt;
...
function isAggregateOnly(sql) {
    ...
    if (/\b(UNION|GROUP\s+BY|HAVING)\b/i.test(norm))
        return false;
    ...
    if (items.length === 0)
        return false;
    const aggRe = /^(?:COUNT|SUM|MIN|MAX|AVG)\s*\(\s*(?:DISTINCT\s+)?(?:\*|[A-Z0-9_./~]+)\s*\)(?:\s+AS\s+[A-Z0-9_]+)?$/i;
...
function matchOne(name, profile) {
    if (profile === 'off')
        return null;
...
function checkTables(names) {
    ...
    for (const raw of names) {
        ...
        seen.add(upper);
        if (allow.has(upper)) {
            process.stderr.write(`[mcp-abap-adt][blocklist] AUDIT: MCP_ALLOW_TABLE bypass for ${upper}\n`);
            continue;
        }
 ...
 */
function evaluateHits(hits, acknowledgeRisk, profile = readProfile()) {
    if (hits.length === 0)
        return { kind: 'pass' };
...
//# sourceMappingURL=tableBlocklist.js.map