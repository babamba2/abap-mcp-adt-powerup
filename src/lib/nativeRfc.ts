...
type NodeRfcPool = {
  acquire: () => Promise<NodeRfcClient>;
  release: (c: NodeRfcClient) => Promise<void>;
};
...
let pool: NodeRfcPool | null = null;
...
function getPool(): NodeRfcPool {
  ...
  try {
    ...
    loadErr = new Error(
      `node-rfc module could not be loaded. Install the SAP NW RFC SDK 7.50+ ` +
  ...
  try {
    ...
    loadErr = new Error(
      `node-rfc Pool could not be initialised. Check that libsapnwrfc ` +
        `is resolvable (SAPNWRFC_HOME) and that SAP_RFC_* credentials are ` +
        `complete in sap.env. Original error: ${(e as Error).message}`,
    );
    throw loadErr;
  }
  return pool;
...
function readConnectionParams(): Record<string, string> {
  ...
  if (mshost) {
    ...
    if (process.env.SAP_RFC_MSSERV) base.msserv = process.env.SAP_RFC_MSSERV;
  } else {
  ...
  if (sncQop) {
    ...
    if (process.env.SAP_RFC_SNC_LIB) base.snc_lib = process.env.SAP_RFC_SNC_LIB;
  }
