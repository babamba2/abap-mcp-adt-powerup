 ...
 */
export function resolveAdtUri(args: ResolveUriArgs): string {
  ...
  switch (type) {
    ...
    case 'FUGR/FF':
      ...
      return `/sap/bc/adt/functions/groups/${encodeSapObjectName(args.parentName).toLowerCase()}/fmodules/${lowerName}`;
    case 'FUGR/I':
      if (!args.parentName) {
        throw new Error(
      ...
      return `/sap/bc/adt/functions/groups/${encodeSapObjectName(args.parentName).toLowerCase()}/includes/${lowerName}`;
    ...
    default:
      ...
      throw new Error(
        `resolveAdtUri: no URI mapping for type="${args.type}" (object="${name}"). ` +
