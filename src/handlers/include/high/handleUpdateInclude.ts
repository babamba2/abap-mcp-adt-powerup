...
export async function handleUpdateInclude(
  ...
  try {
    ...
    if (shouldActivate) {
      ...
      const activationResponse = await makeAdtRequestWithTimeout(
        connection,
      ...
      if (
        ...
        const messages =
          parsed?.messages?.msg ?? parsed?.['chkl:messages']?.msg ?? [];
        ...
        const activationErrors: Array<{
          type: string;
          text: string;
          line?: string | number;
          href?: string;
        }> = [];
        const activationWarnings: typeof activationErrors = [];
        ...
        for (const msg of msgArray) {
          ...
          if (msgType === 'E') activationErrors.push(entry);
          else if (msgType === 'W') activationWarnings.push(entry);
        }
        ...
        if (activationErrors.length > 0) {
          ...
          const err: any = new Error(
            `Include ${includeName} activation failed (${activationErrors.length} error${
